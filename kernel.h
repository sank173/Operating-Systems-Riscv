...
// strip away the padding to the struct 
struct virtq_desc{
      uint64_t addr;
      uint32_t len;
      uint16_t flags;
...
struct virtio_virtq{
    struct virtq_desc decs[VIRTQ_ENTRY_NUM];
    struct virtq_avail avial;
...
struct virtio_blk_req{
    ...
    uint32_t reserved;
    uint64_t sector;
      //second descriptor :writable  by the device if it is a read option(VIRTIQ_DESC_F_WRITE)
    uint8_t data[512];
